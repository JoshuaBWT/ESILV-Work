...
function getYearIfWrong(url, body, parameters, callback)
{
  //On tombe sur une page du style http://www.lacentrale.fr/cote-voitures-jeep----.html
    ...
    if($('[href*=' + "cote-voitures-" + parameters.brand + "-" + parameters.model + ']').length === 0)
    {
      ...
      {
        //options[i].replace(/\+/g, "");
        var jqry = parameters.brand + "-" + options[i];
        if($('[href*=' + jqry + '--]').length !== 0)
        {
          url2 = "http://www.lacentrale.fr/" + $('[href*=' + parameters.brand + "-" + options[i] + '--]').attr("href");
    ...
    function(err, response, body)
    {
        ...
        $ = cheerio.load(body);
        var jqry = "cote-voitures-" + parameters.brand + "-" + parameters.model;
        //jqry.replace(/\+/g, "\\\+")
        $('[href*=' + jqry + ']').each(function ()
        {
          var curUrl = "http://www.lacentrale.fr/" + $(this).attr("href");
          var index = curUrl.indexOf(parameters.model) + parameters.model.length + 1;
     ...
     try
     {
      ...
      function(err, response, body) {
        ...
        if(err) {
          ...
          {
            getYearIfWrong(url, body, parameters, function(nbody, nparameters, urls)
            {
                $ = cheerio.load(nbody);
                ...
                if($('.tx12 > span').length !== 0)
                {
                  var result = {};
                  result.pages = Array();
                  result.pages.push({ "url": urls[0], "name": "true Story" });
          ...
          {}
          //si on est sur un modèle ou manque l'annee
          else
            getOptionsList($, function(resultF)
            {
              result = resultF;
